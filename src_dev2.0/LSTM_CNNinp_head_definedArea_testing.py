...
log_directory = r'..\training\logs_dev2\%s_%s_%s_%s_%s_%s_LSTM_tr%s_LSTMCNNinp' %(targetvar, def_epochs, lr_rate ,batchSize, hidden_size, num_layers, trainsize)
log_dir_fig = r'..\training\logs_dev2\%s_%s_%s_%s_%s_%s_LSTM_tr%s_LSTMCNNinp\figures' %(targetvar, def_epochs, lr_rate ,batchSize, hidden_size, num_layers, trainsize)
...
CNN_input = xr.open_dataset(r'%s\full_pred_denorm_UNet.nc'%temp_model_output)
cnn_data = CNN_input.to_array().values
#reshape the data to match the CNN input
cnn_data = cnn_data.reshape(cnn_data.shape[1], cnn_data.shape[0], cnn_data.shape[2], cnn_data.shape[3])
...
X = np.concatenate((X_all, cnn_data), axis=1)
...
y_pred_full_nc.to_netcdf(r'%s\full_pred_denorm_LSTMUNet.nc'%temp_model_output)
