...
testing_path = 'data/testing_random_sampling_fulltile_%s_101010_deltawtd'%area #path to folders with training,testing, validation data
if not os.path.exists(testing_path):
    os.makedirs(testing_path)

print('create log directory for tensorboard logs')
sys.stdout.flush()
log_directory = 'results/testing/random_sampling_fulltile_%s_101010_limitedInpSel_deltawtd/%s_%s_%s_%s_%s' %(area, model_type, epochs, learning_rate, batch_size, samplesize)
log_dir_fig = 'results/testing/random_sampling_fulltile_%s_101010_limitedInpSel_deltawtd/%s_%s_%s_%s_%s/plots' %(area, model_type, epochs, learning_rate, batch_size, samplesize)
...
if os.path.exists(testing_path+'/tar_validation_norm_arr.npy'):
    ...
    sys.stdout.flush()
    train_loader, validation_loader, test_loader = data.RS_reload_data_deltawtd(testing_path, batch_size)
else:
    print('data testing folder not prepared - prepare data')   
    sys.stdout.flush()
    train_loader, validation_loader, test_loader = data.RS_cnn_data_prep_target_deltawtd(cnn_sample_path, limitedInpSel, testing_path, batch_size)
...
# mean = np.load('/eejit/home/hausw001/HybGGM/hybGGM_test/data/testing_random_sampling_fulltile_180_inclwtd/full_out_var_mean.npy')
# std = np.load('/eejit/home/hausw001/HybGGM/hybGGM_test/data/testing_random_sampling_fulltile_180_inclwtd/full_out_var_std.npy')
...
for ss in sub_samples[:]:
    ...
    target_norm = np.load('%s/full_target_deltawtd_subsample_%s.npy' %(samplepath, ss))
