...
filename = 'random_samples_fulltile_inclwtd.slurm'
outputname = 'random_samples_fulltile_inclwtd.out'
errorname = 'random_samples_fulltile_inclwtd.err'
jobname = 'random_samples_fulltile_inclwtd'
        ...
        for j in [4, 8 , 16][:]:
            for k in [0.0001][:]:
                for area in [180, 360][:1]:
                    for sampsize in [1][:]:
                        f.write('echo "UNet2 with %s %s %s %s %s start at $(date)" \n' % (sampsize, i, k, j, area))
                        f.write('python src/model/run_hyperparam_testing_random_areas_full_tile.py %s %s %s %s %s UNet2\n' % (sampsize, i, k, j, area))
                        f.write('echo "UNet2 with %s %s %s %s %s done at $(date)" \n' % (sampsize, i, k, j, area))
