...
def hyperparam_tuning_training(X_norm, y_norm, testSize, trainSize, def_epochs, lr_rate, batchSize, mt, save_path, area, log_directory):
...
def hyperparam_tuning_prediction(batch_size, model_type, save_path, log_directory):
    print('load validation data')
    X_val = np.load('%s/X_val.npy' %save_path)
    mask_val = np.load('%s/mask_val.npy'%save_path)
    y_val = np.load('%s/y_val.npy'%save_path)
    ...
    if model_type == 'UNet2':
        ...
        y_pred_raw = run_model_on_full_data(model_reload, validation_loader)
    if model_type == 'UNet4':
    ...
    print('save %s/y_pred_denorm_new.npy' %log_directory)
    np.save('%s/y_pred_denorm_new.npy' %log_directory, y_pred_denorm)
    np.save('%s/y_pred_raw_new.npy' %log_directory, y_pred_raw)
    print('Hyperparameter tuning prediction finished')	



''''random samples''' 
def RS_hyperparam_tuning_training(train_loader, test_loader, val_loader, mt,  lr_rate, def_epochs, log_dir, patience, device, bs, sub_batch_size,writer=None):
    if mt == 'UNet6':
        ...
        train.RS_train_test_model(model, train_loader, test_loader, lr_rate, def_epochs, log_dir, patience, device, bs, sub_batch_size,writer=writer)
        train.RS_val_model(model, val_loader,  device,  bs, sub_batch_size, writer=writer)
        del model
    if mt == 'UNet2':
        writer = SummaryWriter(log_dir=log_dir)
        model = UNet2(input_channels=21, output_channels=1)
...
def RS_run_model_on_full_data(model, data_loader, device):
    model.eval()
    all_outputs = []
    with torch.no_grad():
        for inp, tar, mask in data_loader:
...
def RS_hyperparam_tuning_prediction(val_loader, mt, log_dir, data_dir, device):
    if mt == 'UNet2':
        print('reload model')
        model_reload = UNet2(input_channels=21, output_channels=1)
        ...
        print('run prediction')
        y_pred_raw = RS_run_model_on_full_data(model_reload, val_loader, device) #this is now the delta wtd
    if mt == 'UNet6':   
        print('reload model')
        model_reload = UNet6(input_channels=21, output_channels=1)
        model_reload.load_state_dict(torch.load(os.path.join(log_dir, 'best_model.pth')))
    ...
    if mt == 'UNet4':
        ...
        model_reload.to(device)
        print('run prediction')
        y_pred_raw = RS_run_model_on_full_data(model_reload, val_loader, device)
    out_var_test_mean = np.load('%s/out_validation_var_mean.npy' %data_dir)
    out_var_test_std = np.load('%s/out_validation_var_std.npy'%data_dir)
    y_pred_denorm = y_pred_raw*out_var_test_std[0] + out_var_test_mean[0] 
    np.save('%s/y_pred_denorm_new.npy' %log_dir, y_pred_denorm)
    np.save('%s/y_pred_raw_new.npy' %log_dir, y_pred_raw)
    print('Hyperparameter tuning prediction finished')	

'''random samples cpu'''
def RS_cpu_hyperparam_tuning_training(train_loader, test_loader, val_loader, mt,  lr_rate, def_epochs, log_dir, patience, writer=None):
    if mt == 'UNet6':
        writer = SummaryWriter(log_dir=log_dir)
        model = UNet6(input_channels=21, output_channels=1)
        train.RS_cpu_train_test_model(model, train_loader, test_loader, lr_rate, def_epochs, log_dir, patience, writer=writer)
        train.RS_cpu_val_model(model, val_loader, writer=writer)
        del model
    if mt == 'UNet2':
        writer = SummaryWriter(log_dir=log_dir)
        model = UNet2(input_channels=21, output_channels=1)
...
def RS_cpu_run_model_on_full_data(model, data_loader):
    model.eval()
    all_outputs = []
    with torch.no_grad():
        for inp, tar, mask in data_loader:
...
def RS_cpu_hyperparam_tuning_prediction(val_loader, mt, log_dir, data_dir):
    if mt == 'UNet2':
        model_reload = UNet2(input_channels=21, output_channels=1)
        model_reload.load_state_dict(torch.load(os.path.join(log_dir, 'best_model.pth')))
        y_pred_raw = RS_cpu_run_model_on_full_data(model_reload, val_loader) #this is now the delta wtd
    if mt == 'UNet6':   
        model_reload = UNet6(input_channels=21, output_channels=1)
        model_reload.load_state_dict(torch.load(os.path.join(log_dir, 'best_model.pth')))
        y_pred_raw = RS_cpu_run_model_on_full_data(model_reload, val_loader) #this is now the delta wtd
    if mt == 'UNet4':   
        model_reload = UNet4(input_channels=21, output_channels=1)
        model_reload.load_state_dict(torch.load(os.path.join(log_dir, 'best_model.pth')))
        y_pred_raw = RS_cpu_run_model_on_full_data(model_reload, val_loader)
    out_var_test_mean = np.load('%s/out_validation_var_mean.npy' %data_dir)
    out_var_test_std = np.load('%s/out_validation_var_std.npy'%data_dir)
    y_pred_denorm = y_pred_raw*out_var_test_std[0] + out_var_test_mean[0] 
    np.save('%s/y_pred_denorm_new.npy' %log_dir, y_pred_denorm)
...
def RS_full_prediction(full_run, mt, log_dir, data_dir, device):
    if mt == 'UNet2':
        print('reload model')
        model_reload = UNet2(input_channels=21, output_channels=1)
        model_reload.load_state_dict(torch.load(os.path.join(log_dir, 'best_model.pth')))
        model_reload.to(device)
        print('run prediction')
        y_pred_raw = RS_run_model_on_full_data(model_reload, full_run, device) #this is now the delta wtd
    if mt == 'UNet6':   
        print('reload model')
        model_reload = UNet6(input_channels=21, output_channels=1)
        model_reload.load_state_dict(torch.load(os.path.join(log_dir, 'best_model.pth')))
    ...
    out_var_test_mean = np.load('%s/full_out_var_mean.npy' %data_dir)
    out_var_test_std = np.load('%s/full_out_var_std.npy'%data_dir)
    y_pred_denorm = y_pred_raw*out_var_test_std[0] + out_var_test_mean[0] 
    np.save('%s/yfull_pred_denorm_new.npy' %log_dir, y_pred_denorm)
    np.save('%s/yfull_pred_raw_new.npy' %log_dir, y_pred_raw)
    print('full prediction finished')	