...
full_tile_subsample_path = '../../data/testing_random_sampling_fulltile_%s_101010_deltawtd/subsamples'%area
...
if os.path.exists('%s/full_input_globgm-wtd_subsample_99.npy' % (full_tile_subsample_path)):
    ...
    for param in limitedInpSel[:]:
            ...
            if param == 'globgm-wtd':
                ...
                np.save('%s/full_target_deltawtd.npy' %(testing_path), target)
                print('target shape', target.shape) 
                ...
                for n, m in zip(n_samples[:], m_samples[:]):
                    ...
                    # print(sample.shape)
                    np.save('%s/full_target_deltawtd_subsample_%s.npy' %(full_tile_subsample_path, m), sample)
            else:
