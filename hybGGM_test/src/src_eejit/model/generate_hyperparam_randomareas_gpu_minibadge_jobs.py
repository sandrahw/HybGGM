...
filename = 'hyperparam_small_randomarea_360_gpu_U6_minibadge.slurm'
outputname = 'hyperparam_small_randomarea_360_gpu_U6_minibadge.out'
errorname = 'hyperparam_small_randomarea_360_gpu_U6_minibadge.err'
jobname = 'hyperparam_small_randomarea_360_gpu_U6_minibadge'
...
with open('%s'%filename, 'w') as f:
    ...
    for i in [10][:]:
        for j in [ 4,8,32,64,128,256][:]:
            for k in [0.0001][:]:
                for area in [180, 360][1:]:
                    for sampsize in [1][:]:
                        f.write('echo "UNet6 with %s %s %s %s %s start at $(date)" \n' % (sampsize, i, k, j, area))
                        f.write('python src/model/run_hyperparam_testing_random_areas_minibadge.py %s %s %s %s %s UNet6\n' % (sampsize, i, k, j, area))
