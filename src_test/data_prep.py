...
# lon_bounds = (input_rch.lon.min().values, input_rch.lon.max().values)
# lat_bounds = (input_rch.lat.min().values, input_rch.lat.max().values)
lon_bounds = (0, 15)
lat_bounds = (45, 60)

target_wtd_cropped = target.sel(lat=slice(*lat_bounds), lon=slice(*lon_bounds))
...
target_wtd_cropped.to_netcdf(r'..\data\temp\wtd.nc')
...
for param in params[:]:
    data = glob.glob(r'%s\%s\transient\netcdf_maps\%s2*.nc' %(input_path, tile, param))
